...
ptest_res_t test_obj_run_phys() {
  pobj_t o = pobj_create((pvec_t){0}, (pvec_t){100,100,100},
                         1, 0.1);
  phys_t a = phys_create(0, (pvec_t){0,-10,0}, (pvec_t){0},
                         &o, 1, 0);
  phys_run(&a, 0.1, 200);
  if (dist((pvec_t){2000,0,2000}, o.pos) < 1)
    return DONE;
...
ptest_res_t test_100_obj_run_phys() {
  ...
  for (int i = 0; i < OBJ_NUM; i++) {
    o[i] = pobj_create(
      ((pvec_t){0}),
      ((pvec_t){i * 100, i * 100, i * 100}), 
      1, 
      0.1
    );
  }
  phys_t a = phys_create(
    0, 
    (pvec_t){0,-10,0}, 
    (pvec_t){0},
    o, 
...
ptest_res_t test_air() {
  pobj_t o = pobj_create(((pvec_t){0}), ((pvec_t){100,100,100}),
                         1, 0.1);
  phys_t a = phys_create(1.225L, ((pvec_t){0,-10,0}),
                         ((pvec_t){0}), &o, 1, 0);
  phys_run(&a, 0.1, 200);
...
ptest_res_t test_err() {
  ...
  a = phys_create(1.225, (pvec_t){0, -9.8, 0}, (pvec_t){-1, -1, -1},
            &o, 1, 0);
...
ptest_res_t test_gravity() {
  ...
  phys_t phys = phys_create(0, (pvec_t){0}, (pvec_t){0},
                            objs, 3, 1);
...
ptest_t phys_test[] = {
    test_obj_run, test_obj_run_phys, test_100_obj_run_phys,
    test_air,     test_err,          test_gravity,
};
